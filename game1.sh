...
VERSION="1.0"
AUTHOR="Alp GULER , Veli DONMEZ "         
DATE="12.06.2018"
HELP="to Run:\
 ${BOLD}${0}${NORMAL} ${ITALIC}<yourFile>${NORMAL} ${ITALIC}<opponentFile>${NORMAL} [${ITALIC}<username>${NORMAL}];\
 ${BOLD}${0}${NORMAL} [${ITALIC}OPTIONS${NORMAL}] $NORMAL    
    OPTIONS:
        -h - show this message
        -v - version of program
Author: $AUTHOR"                                    #Help part show to users how to start a game
...
for ((i=1;i<=$#;i++)); do
    ...
    if [[ $i == 1 ]]; then 
        USERFILEPATH="/tmp/${!i}"                                                     #creating path
    fi
    if [[ $i == 2 ]]; then
...
#displaying if its impossible to create file
    return 1
fi
